            ...
            if not selected_todate == None and not selected_fromdate == None:
                if not seleted_category == "none":
                    ...
                    data = Tool.objects.filter(
                        owner=request.user, category=seleted_category, to_date__lte=selected_fromdate)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not selected_location == "none":
                ...
                edit = 'edit'
                return render(request, 'tools/add_tools.html', {'loc': loc, 'data1': data1, 'form': form, 'edit': edit})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not selected_location == "none":
                loc = Location.objects.get(id=selected_location)
                form = MapToolCreationForm()
                return render(request, 'tools/add_tools.html', {'loc': loc, 'data1': data1, 'form': form})
...
def owner_payment(request):
    data = Payment.objects.filter(
        order__tools__owner=request.user, status='success')
...
def rating(request, id):
    if request.method == 'POST':
        ...
        Rating.objects.create(customer=request.user, payment=obj,
                              rating=selected_rating, review=selected_review)
